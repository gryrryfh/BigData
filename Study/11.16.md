...
noon_data =data[pd.to_datetime(data.timestamp).dt.hour< 9]
...
HeatMap(data=morning_joined_data[['geo.latitude', 'geo.longitude','people_out']]).add_to(morning_seoul_out)[Uploading seoul-metro-station-info.csv…]()
